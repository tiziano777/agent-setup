...
app = FastAPI(
    title="rlm_agent API",
    description="REST API for rlm_agent - RLM recursive reasoning",
    version="0.1.0",
)
...
async def invoke(req: InvokeRequest):
    """Invoke rlm_agent with messages.

    Example:
        curl -X POST http://localhost:8000/invoke \
            -H "Content-Type: application/json" \
            -d '{"messages": [{"role": "user", "content": "Find answer in this long context..."}]}'
    """
    try:
